...
- **複数APIキー対応**：最大10個のAPIキーを設定可能
- **自動キー切り替え**：1つのキーが上限に達したら自動的に次のキーに切り替え
- 検索結果をK列（店舗番号）に自動記入
- **K列に既にデータがある行はスキップ**（既存データ保護）
- **検索結果のプレビュー表示**（店舗名、都道府県、電話番号）
- **統計情報表示**（検索件数、スキップ件数、利用可能なAPIキー数）
...
複数のAPIキーを設定すると、1つのキーが上限に達した際に自動的に次のキーに切り替わります。
...
**Streamlit Cloudの場合：**
- アプリの「⋮」→「Settings」→「Secrets」タブで以下を設定：
```toml
SERPAPI_KEY = "あなたのAPIキー1"
SERPAPI_KEY2 = "あなたのAPIキー2"
SERPAPI_KEY3 = "あなたのAPIキー3"
...
- **複数のAPIキーを設定することで検索回数を増やせます**（例：3つのキー = 300回/月）
- 1つのAPIキーが上限に達すると、**自動的に次のキーに切り替わります**
- 全てのAPIキーが上限に達した場合、「全てのAPIキーが上限に達しました」と表示されます
